...
const static char* fmtSignBmp = "BM";
...
bool BmpDecoder::readHeader()
{
    ...
    try
    {
        ...
        HL_Assert(size > 0);    // overflow, 2Gb limit
        ...
        if (size >= 36)
        {
            ...
            HL_Assert(m_rle_code_ >= 0 && m_rle_code_ <= BMP_BITFIELDS);
            ...
            if (m_width > 0 && m_height != 0 && (((m_bpp == 1 || m_bpp == 4 || m_bpp == 8 || m_bpp == 24 || m_bpp == 32) && m_rle_code == BMP_RGB) || ((m_bpp == 16 || m_bpp == 32) && (m_rle_code == BMP_RGB || m_rle_code == BMP_BITFIELDS)) || (m_bpp == 4 && m_rle_code == BMP_RLE4) || (m_bpp == 8 && m_rle_code == BMP_RLE8)))
            {
                iscolor = true;
                result  = true;
                ...
                if (m_bpp <= 8)
                {
                    HL_Assert(clrused >= 0 && clrused <= 256);
        ...
        else if (size == 12)
        {
            ...
            if (m_width > 0 && m_height != 0 && (m_bpp == 1 || m_bpp == 4 || m_bpp == 8 || m_bpp == 24 || m_bpp == 32))
            {
                if (m_bpp <= 8)
                {
                    uchar buffer[256 * 3];
    ...
    // in 32 bit case alpha channel is used - so require HL_8UC4 type
    m_type   = iscolor ? ((m_bpp == 32 && m_rle_code != BMP_RGB) ? HL_8UC4 : HL_8UC3) : HL_8UC1;
...
bool BmpDecoder::readData(Mat& img)
{
    ...
    HL_Assert(((uint64)m_height * m_width * nch < (HL_BIG_UINT(1) << 30)) && "BMP reader implementation doesn't support large images >= 1Gb");
        ...
        switch (m_bpp)
        {
            ...
            case 1 :
                for (y = 0; y < m_height; y++, data += step)
                {
                    ...
                    if (!color)
                        ihlCvt_BGR2Gray_8u_C3C1R(bgr, 0, data, 0, Size(m_width, 1));
                }
                result = true;
                break;
            ...
            case 15 :
                for (y = 0; y < m_height; y++, data += step)
                {
                    m_strm.getBytes(src, src_pitch);
                    if (!color)
                        ihlCvt_BGR5552Gray_8u_C2C1R(src, 0, data, 0, Size(m_width, 1));
                    else
                        ihlCvt_BGR5552BGR_8u_C2C3R(src, 0, data, 0, Size(m_width, 1));
                }
                result = true;
                break;
            /************************* 16 BPP ************************/
            case 16 :
                for (y = 0; y < m_height; y++, data += step)
                {
                    m_strm.getBytes(src, src_pitch);
                    if (!color)
                        ihlCvt_BGR5652Gray_8u_C2C1R(src, 0, data, 0, Size(m_width, 1));
                    else
                        ihlCvt_BGR5652BGR_8u_C2C3R(src, 0, data, 0, Size(m_width, 1));
                }
                result = true;
                break;
            /************************* 24 BPP ************************/
            case 24 :
                for (y = 0; y < m_height; y++, data += step)
                {
                    m_strm.getBytes(src, src_pitch);
                    if (!color)
                        ihlCvt_BGR2Gray_8u_C3C1R(src, 0, data, 0, Size(m_width, 1));
                    else
            ...
            case 32 :
            {
                ...
                for (y = 0; y < m_height; y++, data += step)
                {
                    ...
                    if (!color)
                    {
                        if (has_bit_mask)
                            ...
                            ihlCvt_BGRA2Gray_8u_C4C1R(src, 0, data, 0, Size(m_width, 1));
                    }
                    else if (img.channels() == 3)
                    {
                        if (has_bit_mask)
                            ...
                            ihlCvt_BGRA2BGR_8u_C4C3R(src, 0, data, 0, Size(m_width, 1));
                    }
                ...
                break;
            default :
                HL_Error(hl::Error::StsError, "Invalid/unsupported mode");
        }
    }
    catch (...)
    {
