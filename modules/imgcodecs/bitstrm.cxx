...
int RBaseStream::getPos()
{
    ...
    HL_Assert(pos >= m_block_pos);    // overflow check
    HL_Assert(pos >= 0);              // overflow check
    return pos;
}
...
void RBaseStream::skip(int bytes)
{
    ...
    uchar* old  = m_current;
    m_current  += bytes;
    HL_Assert(m_current >= old);    // overflow check
}
...
void WLByteStream::putBytes(const void* buffer, int count)
{
    ...
    HL_Assert(data && m_current && count >= 0);
