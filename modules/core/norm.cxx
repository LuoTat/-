...
int normInf_(const T* src, const uchar* mask, ST* _result, int len, int cn)
{
    ...
    else
    {
        for (int i = 0; i < len; i++, src += cn)
            ...
            {
                for (int k = 0; k < cn; k++)
                    result = std::max(result, ST(hl_abs(double(src[k]))));
            }
    }
    *_result = result;
    return 0;
...
int normDiffL1_(const T* src1, const T* src2, const uchar* mask, ST* _result, int len, int cn)
{
    ...
    else
    {
        for (int i = 0; i < len; i++, src1 += cn, src2 += cn)
            ...
            {
                for (int k = 0; k < cn; k++)
                    result += std::abs(double(src1[k] - src2[k]));
            }
    }
    *_result = result;
    return 0;
...
static NormFunc getNormFunc(int normType, int depth)
{
    static NormFunc normTab[3][8] = {
        {(NormFunc)normInf_8u, (NormFunc)normInf_8s, (NormFunc)normInf_16u, (NormFunc)normInf_16s, (NormFunc)normInf_32u, (NormFunc)normInf_32s, (NormFunc)normInf_32f, (NormFunc)normInf_64f},
        {(NormFunc)normL1_8u, (NormFunc)normL1_8s, (NormFunc)normL1_16u, (NormFunc)normL1_16s, (NormFunc)normL1_32u, (NormFunc)normL1_32s, (NormFunc)normL1_32f, (NormFunc)normL1_64f},
        {(NormFunc)normL2_8u, (NormFunc)normL2_8s, (NormFunc)normL2_16u, (NormFunc)normL2_16s, (NormFunc)normL2_32u, (NormFunc)normL2_32s, (NormFunc)normL2_32f, (NormFunc)normL2_64f}};

    return normTab[normType][depth];
}

double norm(const Mat& _src, int normType, Mat& _mask)
{
