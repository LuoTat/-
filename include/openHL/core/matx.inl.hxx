...
template <typename _Tp>
inline Vec<_Tp, 2> conjugate(const Vec<_Tp, 2>& v)
{
    return Vec<_Tp, 2>(v[0], -v[1]);
}

template <typename _Tp>
inline Vec<_Tp, 4> conjugate(const Vec<_Tp, 4>& v)
{
...
inline static MatxCommaInitializer<_Tp, m, n> operator<<(const Matx<_Tp, m, n>& mtx, _T2 val)
{
...
template <typename _Tp, int m, int n>
inline MatxCommaInitializer<_Tp, m, n>::MatxCommaInitializer(Matx<_Tp, m, n>* _mtx):
    dst(_mtx), idx(0)
{}

template <typename _Tp, int m, int n> template <typename _T2>
inline MatxCommaInitializer<_Tp, m, n>&MatxCommaInitializer<_Tp, m, n>::operator,(_T2 value)
{
    HL_DbgAssert(idx < m * n);
...
template <typename _Tp, int m, int n>
inline Matx<_Tp, m, n>::diag_type Matx<_Tp, m, n>::diag() const
{
    diag_type d;
    for (int i = 0; i < shortdim; i++)
        d.val[i] = val[i * n + i];
...
{
    return hl::internal::Matx_DetOp<_Tp, m>()(a);
}
...
inline static VecCommaInitializer<_Tp, cn> operator<<(const Vec<_Tp, cn>& vec, _T2 val)
{
